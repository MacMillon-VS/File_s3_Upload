...
async function s3Upload (req, res) {
    ...
    try{
        await uploadFileToS3(formData.file, 'file-handling-test-bucket');
        res.send('Uploaded!!');
    } catch(ex) {
        res.send('ERROR!!!!');
    }
...
async function s3Get (req, res) {
    try{
        const bucketData = await getBucketListFromS3('file-handling-test-bucket');
...
async function getSignedUrl(req, res) {
    try {
        ...
        const url = await getPresignedURL('file-handling-test-bucket', key);
        res.send(url);
